(function(root,factory){
  ...
  const EVENT_TYPES=new Set(["military_exercise","deployment","air_policing_activity","readiness_increase","drone_incident","border_incident","naval_exercise","official_defence_announcement"]);
  ...
  function normalizeEvent(raw){
    ...
    const confirmed=officialUrl(sourceUrl)&&clean(raw.confidence,30).toUpperCase()==="CONFIRMED";
    const discovery=raw.discovery&&typeof raw.discovery==="object"?{temporaryWatchZone:raw.discovery.temporaryWatchZone===true,score:Math.max(0,Math.min(100,Number(raw.discovery.score)||0)),scoreReasons:list(raw.discovery.scoreReasons,20),scoreExplanation:clean(raw.discovery.scoreExplanation,500)}:null;
  ...
  return {EVENT_TYPES:[...EVENT_TYPES],ACTIVITY_TYPES:[...ACTIVITY_TYPES],OFFICIAL_HOSTS,clean,safeUrl,officialUrl,normalizeEvent,importEvents,statusAt,distanceKm,assessEvent,diffEvents};
});
