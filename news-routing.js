(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.NordicWatchNewsRouting=api})(globalThis,function(){
  ...
  const text=v=>String(v||'').normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/ł/g,'l').replace(/[æÆ]/g,'ae').replace(/[øØ]/g,'o').toLowerCase();
  const time=v=>v?+new Date(v):NaN;
  const regions=[['Sweden','sweden|swedish|gotland'],['Finland','finland|finnish'],['Norway','norway|norwegian|finnmark'],['Denmark','denmark|danish|danmark|dansk|bornholm|gedser'],['Iceland','iceland|icelandic'],['Estonia','estonia|estonian|narva'],['Latvia','latvia|latvian|namejs'],['Lithuania','lithuania|lithuanian|lietuva|lietuvos|lietuvoje|kaunas|kauno|kaisiadorys|kaisiadoriu'],['Polish Baltic coast','polish baltic coast|gdansk|gdynia|szczecin|suwalki'],['Northeastern Poland','masurian patrol|bemowo piskie|masuria'],['Baltic Sea','baltic|gulf of finland'],['Kaliningrad / Baltijsk','kaliningrad|konigsberg|koenigsberg|baltijsk|baltiysk'],['Kola / Murmansk','kola|murmansk|severomorsk'],['Barents / Svalbard','barents|svalbard|spitsbergen|barentsburg'],['Jan Mayen / High North','jan mayen|high north'],['Northern Germany','schleswig[ -]holstein|mecklenburg[ -]vorpommern|kiel|rostock|lubeck|luebeck|rugen|ruegen|fehmarn|greifswald|stralsund|german baltic ports?|hamburg']];
  const topics=[['MILITARY_EXERCISE','exercise|training|namejs|airshow'],['MILITARY_ACTIVITY','military|militaer|defence|defense|forsvar|naval|navy|fregat|aircraft|helikopter|intercept(?:ion|ed|s)?|isr|awacs|fighters?|naikintuv\w*|oro erdv\w*|deployments?|airlift|submarines?|warships?|missiles?|troops'],['INFRASTRUCTURE','infrastructure|cable|pipeline|railway|telecom|power grid|energy'],['HYBRID_SECURITY','cyber|sabotage|drone|dron\w*|bepilot\w*|gnss|jamming|security'],['POLITICAL_SECURITY','sanctions|seizure|diplomatic|rhetoric|protest|nato|molchanov']];
  ...
  const nordicDemonym=/\b(?:swedish|finnish|norwegian|danish|dansk|icelandic|estonian|latvian|lithuanian|lietuvisk\w*)\b/;
  ...
  function evaluateNordicRelevance(item){
    ...
    const extractedCountries=[...new Set(content.match(/\b(?:poland|polish|sweden|swedish|finland|finnish|norway|norwegian|denmark|danish|danmark|dansk|iceland|estonia|latvia|lithuania|lietuva|lietuvos|lietuvoje)\b/g)||[])];
  ...
  function normalize(item,now=Date.now()){
    ...
    const evidence=/^(CONFIRMED|CONFIRMED_EXTERNAL)$/.test(item.confidence)||/CONFIRMED/.test(item.risk?.sourceConfidence)||/(^|\.)(nato\.int|dvidshub\.net|mil|forsvarsmakten\.se|mil\.lv|forsvaret\.dk|kam\.lt|kariuomene\.lt)$/.test(source)?'CONFIRMED_EXTERNAL':'INFERRED';
