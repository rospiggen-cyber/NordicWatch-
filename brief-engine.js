(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.NordicWatchBrief=api})(globalThis,function(){
  ...
  function build({articles=[],situations=[],signals=[],previous=null,now=Date.now(),geographic=()=>true,coverage=null}={}){const news=selectNews(articles,{now,situations,geographic}),coverageAssessment=assessCoverage(coverage,news.counters),developments=[];
    ...
    developments.sort((a,b)=>b.score-a.score);return {developments:developments.slice(0,3),news,coverageAssessment,watching:situations.filter(s=>['DEVELOPING','ELEVATED','HIGH'].includes(s.level)),snapshot:{t:now,situations:situations.map(s=>({situationId:s.situationId,signalIds:s.signalIds,situationScore:s.situationScore}))}};
  }
