(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.NordicWatchBrief=api})(globalThis,function(){
  ...
  function build({articles=[],situations=[],signals=[],upstreamTriggers=[],previous=null,coverage={},now=Date.now(),geographic=()=>true}={}){const news=selectNews(articles,{now,situations,geographic}),coverageAssessment=assessCoverage(coverage,news.counters),developments=[],core=typeof globalThis.NordicWatchIntelligence==='object'?globalThis.NordicWatchIntelligence:typeof require==='function'?require('./intelligence-core.js'):null,triggerWatch=core?.watchDirectives(upstreamTriggers,now)||[];
