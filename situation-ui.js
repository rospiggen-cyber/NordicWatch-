...
function refreshSituations(force=false){
  ...
  try{
    ...
    for(const a of readArray('NORDICWATCH_SIGNAL_ARCHIVE')){const p=a.risk?.places?.[0];if(!NR.safeRelevance(a).relevant)continue;incoming.push({id:a.signalId||a.id,incidentId:a.eventId,domain:'NEWS',timestamp:a.temporal?.containsNewDevelopment?a.lastUpdatedAt:a.eventTime||a.ingestedAt,lat:p?.lat,lon:p?.lon,region:situationRegion(p?.lat,p?.lon,p?.name),title:a.title,description:a.description,source:a.domain,sourceUrl:a.url,evidence:a.evidenceType==='CONFIRMED_EXTERNAL'||/CONFIRMED/.test(a.risk?.sourceConfidence)?'CONFIRMED EXTERNAL REPORT':'AUTOMATIC ASSESSMENT',quality:a.timeStatus==='TIME_UNCERTAIN'?35:a.risk?.components?.sourceConfidence,eventScore:a.risk?.score,entities:a.entities})}
    for(const e of allEvents){if(!NR.safeRelevance(e).relevant)continue;incoming.push({id:'event-'+e.id,incidentId:readArray('NORDICWATCH_SIGNAL_ARCHIVE').find(a=>IC.canonical(a.url)===IC.canonical(e.sourceUrl))?.eventId||e.id,domain:'OFFICIAL',timestamp:e.lastObservedAt||e.temporal?.articlePublishedAt||e.startTime,lat:e.latitude,lon:e.longitude,region:situationRegion(e.latitude,e.longitude,e.areaName),title:e.title,description:e.description,source:new URL(e.sourceUrl).hostname,sourceUrl:e.sourceUrl,evidence:e.confidence==='CONFIRMED'?'CONFIRMED EXTERNAL REPORT':'AUTOMATIC ASSESSMENT',eventScore:e.discovery?.score,quality:e.confidence==='CONFIRMED'?95:50,entities:e.countries,kind:e.eventType})}
...
function showSituation(s){const body=openExplanation(s.title);explanationText(body,`SituationScore ${s.situationScore} · ${s.level}`,`${s.relationship} · confidence ${s.confidence}. ${s.summary}`);
  explanationText(body,'Why these observations were connected',s.links.map(p=>`${p.a} ↔ ${p.b}: ${p.reason}; ${p.distanceKm==null?'Distance unavailable — no proximity credit':p.distanceKm+' km'} / ${p.hours} h`).join('\n'));
  explanationText(body,'Score contributions',Object.entries(s.components).map(([k,v])=>`${k}: ${v===null?'NOT AVAILABLE (excluded)':v.toFixed(1)}`).join(' · ')+'. Final score is capped for isolated, single-source or routine surveillance evidence.');
  explanationText(body,'Baseline → deviation → CIC',`${s.baseline.status} · ${s.baseline.sampleDays} reference days · deviation ${s.baseline.deviation===null?'not assessable':s.baseline.deviation.toFixed(0)}. Coherence ${s.cic.coherence.toFixed(2)} × Interaction ${s.cic.interaction.toFixed(2)} × Corroboration ${s.cic.corroboration.toFixed(2)} = ${s.cic.score}/100.`);
  if(s.limitations?.length)explanationText(body,'Assessment limitations',s.limitations.join(' '));
  if(s.contributionTrace){const t=s.contributionTrace;explanationText(body,'Accumulation audit',t.accumulation.count+' observations × 6, capped at 30, in the full 7-day group: '+t.accumulation.signalIds.join(', '));explanationText(body,'Source support audit',t.sourceSupport.count+' source labels × 5, capped at 15. '+t.sourceSupport.meaning+' '+t.sourceSupport.groups.map(g=>g.source+': '+g.signalIds.join(', ')).join(' · '));explanationText(body,'CIC geographic audit',t.cic.geographicLinks+' located links; '+t.cic.excludedUnlocatedLinks+' unlocated links excluded from interaction credit.');explanationText(body,'Event context audit','Maximum EventScore '+t.eventContext.maximum+' × '+t.eventContext.weight+' from '+t.eventContext.signalIds.join(', '));}
...
async function generateBrief(kind){
  ...
  const routed=routeNewsEvidence();refreshSituations(true);const previous=maritimeRead('NORDICWATCH_BRIEF_V2',null),brief=BX.build({articles:routed.articles,situations:situationClusters,signals:situationObservations,upstreamTriggers:typeof upstreamTriggerSnapshot==='function'?upstreamTriggerSnapshot():[],previous,coverage:{newsChecked:!routed.degraded&&briefNewsCoverage?.healthy===true,externalChecked:briefExternalCoverage},geographic:a=>NR.safeRelevance(a).relevant});
