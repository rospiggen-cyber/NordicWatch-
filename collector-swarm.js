(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.NordicWatchCollectors=api})(globalThis,function(){
  ...
  function normalizeEvidence(raw,collectorId,now){
    ...
    return {id:evidenceId,collectorId,timestamp,title:clean(raw.title||raw.details||raw.kind||evidenceId),region:clean(raw.region||raw.areaName||raw.location),source:clean(raw.source?.name||raw.source?.publisher||raw.sourceName||raw.source||'UNKNOWN',120),sourceUrl:/^https:\/\//.test(String(raw.sourceUrl||raw.url||raw.link||''))?String(raw.sourceUrl||raw.url||raw.link):null,score:Math.max(0,Math.min(100,Number(raw.score??raw.eventScore??raw.risk?.total??raw.risk?.score)||0)),confidence:clean(raw.confidence||raw.sourceConfidence||raw.evidenceType||'UNKNOWN',60),kind:clean(raw.kind||raw.role||raw.capability||raw.type,80),domain:clean(raw.domain||collectorId,40).toUpperCase(),directAoiEvent:raw.directAoiEvent!==false,watchAreas:unique(raw.watchAreas||[]).map(x=>clean(x,80)),watchTargets:unique(raw.watchTargets||[]).map(x=>clean(x,80))};
  }
  ...
  function defaults(){return createSwarm([
    ...
    {id:'upstream-triggers',title:'External triggers',description:'Confirmed external activity that raises watch priorities without becoming an AOI incident.',windowMs:48*3600000,select:s=>s.upstreamTriggers||[],score:e=>Math.min(74,e.length*12+Math.max(0,...e.map(x=>x.score)))}
  ])}
